...
// Edited on 04/04/22 by Elizabeth Larson
// Includes refreshDeviceCache which is called in the discoverServices method
...
public class FlutterBluePlugin
        ...
        switch (call.method) {
            ...
            case "startScan": {
                ensurePermissionsBeforeAction(
                        ...
                        Manifest.permission.ACCESS_FINE_LOCATION, (granted, permission) -> {
                            if (granted)
                                startScan(call, result);
                            else
                                result.error(
                                        "no_permissions",
                                        String.format("flutter_blue plugin requires %s for scanning", permission),
                                        null);
                        });
                break;
            }
            ...
            case "connect": {
                ensurePermissionBeforeAction(Manifest.permission.BLUETOOTH_CONNECT, null, (granted, permission) -> {
                    if (!granted) {
                        result.error(
                                "no_permissions",
                                String.format("flutter_blue plugin requires %s for new connection", permission), null);
                        return;
                    }
            ...
            case "setNotification": {
                ...
                if (!cccDescriptor.setValue(value)) {
                    result.error("set_notification_error", "error when setting the descriptor value to: " + value,
                            null);
                    return;
                }
            ...
            case "requestMtu": {
                ...
                try {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        ...
                        result.error("requestMtu", "Only supported on devices >= API 21 (Lollipop). This device == "
    ...
    private void ensurePermissionBeforeAction(String permissionA12, String permission,
            OperationOnPermission operation) {
        ensurePermissionsBeforeAction(new String[] { permissionA12 }, permission, operation);
    }

    private void ensurePermissionsBeforeAction(String[] permissionsA12, String permission,
            OperationOnPermission operation) {
        String[] permissions = Build.VERSION.SDK_INT >= Build.VERSION_CODES.S ? permissionsA12
                : permission != null ? new String[] { permission } : null;
                ...
                if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
                    ...
                    switch (state) {
                        ...
                        case BluetoothAdapter.STATE_TURNING_OFF:
                            ...
                            break;
                        case BluetoothAdapter.STATE_ON:
                            sink.success(Protos.BluetoothState.newBuilder().setState(Protos.BluetoothState.State.ON)
                                    .build().toByteArray());
                            break;
                        case BluetoothAdapter.STATE_TURNING_ON:
                            sink.success(Protos.BluetoothState.newBuilder()
